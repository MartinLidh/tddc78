...
void blurfilter(const int xsize, const int ysize, pixel* src, const int radius, 
    const double *w, const int over, const int under, pixel* dst, pixel *temp){
  int x,y,x2,y2, wi;
  double r,g,b,n, wc;
  ...
  for (y=0; y<ysize; y++) {
	...
	if(y2 < ysize+under) {
      ...
      pix(temp, x, y, xsize)->r = r/n;
      pix(temp, x, y, xsize)->g = g/n;
      pix(temp, x, y, xsize)->b = b/n;
    }
  }

 for (y=0; y<ysize; y++) {
    for (x=0; x<xsize; x++) {
      r = w[0] * pix(temp, x, y, xsize)->r;
      g = w[0] * pix(temp, x, y, xsize)->g;
      b = w[0] * pix(temp, x, y, xsize)->b;
      n = w[0];
      for ( wi=1; wi <= radius; wi++) {
	wc = w[wi];
	x2 = x - wi;
	if(x2 >= 0) {
	  r += wc * pix(temp, x2, y, xsize)->r;
	  g += wc * pix(temp, x2, y, xsize)->g;
	  b += wc * pix(temp, x2, y, xsize)->b;
	  n += wc;
	}
	x2 = x + wi;
	if(x2 < xsize) {
