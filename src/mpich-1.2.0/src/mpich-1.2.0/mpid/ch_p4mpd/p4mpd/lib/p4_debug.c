...
#define STDPRT stdout
    if (level > p4_debug_level)
	return;
#ifdef USE_PTHREADS
    fprintf(STDPRT,"%d: %s: %u: ", level, whoami_p4, pthread_self());
#else
    fprintf(STDPRT,"%d: %s: ", level, whoami_p4);
#endif
    if (p4_global)
	fprintf(STDPRT,"(%f) ", p4_usclock());
    else
	fprintf(STDPRT,"(-) " );
#ifdef VPRINTF
    vfprintf(STDPRT, fmt, ap);
#else
    _doprnt(fmt, ap, STDPRT);
#endif
    va_end(ap);
    fflush(STDPRT);
}
